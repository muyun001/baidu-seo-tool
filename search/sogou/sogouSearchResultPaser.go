...
import (
	...
	"github.com/kevin-zx/baidu-seo-tool/search/baidu"
	"net/url"
	"strings"
)
...
// 判断是否是主页
func (ssi *SogouSearchResult) IsHomePage() bool {
	domain := ""
	if ssi.DisplayUrl != "" && !strings.Contains(ssi.DisplayUrl, "...") {
		if strings.Index(ssi.DisplayUrl, "http") >= 0 {
			domain, _ = baidu.GetDomain(ssi.DisplayUrl)
		} else {
			domain, _ = baidu.GetDomain("http://" + ssi.DisplayUrl)
		}
	} else {
		if strings.Contains(ssi.DisplayUrl, "...") {
			if strings.Contains(ssi.DisplayUrl, "?") {
				return false
			}
			td := strings.Replace(ssi.DisplayUrl, "//", "", -1)
			if strings.Contains(td, "/") {
				return false
			}
		}
		_ = ssi.GetSogouRealUrl()
		if ssi.RealUrl != "" {
			domain, _ = baidu.GetDomain(ssi.RealUrl)
		}
	}
	if ssi.RealUrl != "" {
		return strings.HasSuffix(strings.Replace(ssi.RealUrl, "/", "", -1), domain)
	} else if ssi.DisplayUrl != "" {
		return strings.HasSuffix(strings.Replace(ssi.DisplayUrl, "/", "", -1), domain)
	}
